<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:65:"D:\www\tp\public/../application/admin/view/default/deal\edit.html";i:1506675998;s:67:"D:\www\tp\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<form action="<?php echo url(); ?>" method="post" class="form-horizontal">
    ...
    <div class="form-item">
        <label class="item-label">报修地址<span class="check-tips">（用于调转的URL，支持带http://的URL或U函数参数格式）</span></label>
        <div class="controls">
            <input type="text" class="text input-large" name="address" value="<?php echo (isset($info['address']) && ($info['address'] !== '')?$info['address']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">报修电话<span class="check-tips">（用于调转的URL，支持带http://的URL或U函数参数格式）</span></label>
        <div class="controls">
            <input type="number" class="text input-large" name="tel" value="<?php echo (isset($info['tel']) && ($info['tel'] !== '')?$info['tel']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">问题<span class="check-tips">（用于调转的URL，支持带http://的URL或U函数参数格式）</span></label>
        <div class="controls">
            <input type="text" class="text input-large" name="problem" value="<?php echo (isset($info['problem']) && ($info['problem'] !== '')?$info['problem']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">状态<span class="check-tips">（用于调转的URL，支持带http://的URL或U函数参数格式）</span></label>
        <div class="controls">
            待修理<input type="radio" class="" name="status" value="1" <?php echo !empty($info['status']) && $info['status']==1?'checked':''; ?>/>
            已修理<input type="radio" class="" name="status" value="2" <?php echo !empty($info['status']) && $info['status']==2?'checked':''; ?>>
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">排序<span class="check-tips">（用于调转的URL，支持带http://的URL或U函数参数格式）</span></label>
        <div class="controls">
            排序<input type="text" class="text input-large" name="sort" value="<?php echo (isset($info['sort']) && ($info['sort'] !== '')?$info['sort']:''); ?>">
        </div>
    </div>
  <!--  <div class="form-item">
